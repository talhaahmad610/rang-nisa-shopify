# Rang Nisa — Editorial Shopify Theme

## Project overview

Custom Shopify Online Store 2.0 theme optimized for speed and conversion, following a **minimalist editorial aesthetic** inspired by pk.lamaretail.com.

### Design language (MUST follow in all new files)

- **Typography**: Minimalist sans-serif (Inter for body, Jost for headings). Use theme font variables `var(--font-body)` and `var(--font-heading)`.
- **Layout**: 1px borders everywhere, maximum white space, **zero rounded corners** (all border-radius: 0, square buttons), asymmetric grids.
- **Colors**: Mono palette — black, white, grays. All color defaults use `#000000`, `#FFFFFF`, and gray scale. Use CSS variables from `base.css` (e.g., `--color-gray-200`, `--color-black`).
- **Spacing**: 8px base scale via `--space-1` through `--space-12` tokens defined in `base.css`.
- **SEO**: JSON-LD Schema (already in `snippets/seo-tags.liquid`). Use semantic HTML5 tags (`<main>`, `<section>`, `<article>`, `<nav>`, `<header>`, `<footer>`).
- **Images**: Always lazy-load with `loading: 'lazy'`. Use `image_url` + `image_tag` filters.
- **Buttons**: Class `.btn`, `.btn--primary`, `.btn--secondary`, `.btn--text`. Always square (border-radius: 0).
- **Translations**: Every user-facing string must use `{{ 'key' | t }}`. Schema labels use `t:` prefix. Add new keys to `locales/en.default.json` and `locales/en.default.schema.json`.

### What's already built (DO NOT rebuild these)

| File | Status |
|---|---|
| `layout/theme.liquid` | ✅ Done — master layout with SEO meta, JSON-LD, preconnect, semantic HTML |
| `assets/base.css` | ✅ Done — full design system (tokens, grid, buttons, cards, utilities, animations) |
| `config/settings_schema.json` | ✅ Done — mono palette, 0 border-radius defaults, "Rang Nisa" branding |
| `snippets/seo-tags.liquid` | ✅ Done — JSON-LD for Organization, Product, Article, BreadcrumbList, CollectionPage |
| `sections/header.liquid` | ✅ Done — sticky transparent-to-solid, centered logo, mega-menu, mobile drawer |
| `sections/header-announcements.liquid` | ✅ Done — auto-rotating promo slider, pause on hover, dismissable |
| `sections/hero-video-banner.liquid` | ✅ Done — full-viewport video/image, overlay, text overlay with CTA |
| `sections/editorial-collection-grid.liquid` | ✅ Done — asymmetric grid (1 large + 2 small), collection overlays |
| `sections/featured-product-slider.liquid` | ✅ Done — scroll-snap carousel, product cards, quick-add via fetch |
| `locales/en.default.json` | ✅ Updated — new section translation keys added |
| `locales/en.default.schema.json` | ✅ Updated — new schema translation keys added |

### What needs to be built next (IN THIS ORDER)

#### Step 1: Remaining homepage section

1. **`sections/text-with-image-split.liquid`** — Minimalist 50/50 split layout. One side image, other side text with elegant typography. Settings: image, heading, description (richtext), button_label, button_link, image_position (left/right), color_scheme. Use the `.grid--split` class from `base.css`.

#### Step 2: Product page

2. **`sections/main-product.liquid`** — Editorial product page with:
   - Stacked image gallery (not thumbnail carousel — stack all images vertically on left)
   - Product info on right: title, vendor, price, variant picker, quantity, add-to-cart, accordion details (description, shipping, returns)
   - Dynamic checkout button support (`{{ form | payment_button }}`)
   - Use `{% form 'product' %}` for the add-to-cart form
   - Responsive: stack to single column on mobile
   - Breadcrumbs at top using `snippets/seo-tags.liquid` breadcrumb data

3. **`templates/product.json`** — JSON template referencing `main-product` section

#### Step 3: Collection page

4. **`sections/main-collection.liquid`** — Fast-loading collection grid with:
   - Collection title + description header
   - Product grid using `.card` classes from `base.css` (2 cols mobile, 3-4 cols desktop)
   - Sidebar filtering (by price, availability, tags) using Shopify's `{% for filter in collection.filters %}`
   - Sort dropdown
   - Pagination using `{% paginate %}` tag
   - Lazy-load all images

5. **`templates/collection.json`** — JSON template referencing `main-collection` section

#### Step 4: Cart page

6. **`sections/main-cart.liquid`** — Clean, focused cart with:
   - Line items with image, title, variant, price, quantity selector, remove button
   - Cart summary: subtotal, note about shipping/taxes, checkout button
   - Empty cart state with "Continue shopping" link
   - Use `{% form 'cart' %}` for updates

7. **`templates/cart.json`** — JSON template referencing `main-cart` section

#### Step 5: Footer

8. **`sections/footer.liquid`** — Multi-column editorial footer with:
   - Newsletter signup (email input + submit button, using `.newsletter` class from `base.css`)
   - Multiple link columns from menu (link_list settings)
   - Social media icon links
   - Copyright line with payment icons
   - 1px border-top separator
   - Blocks: link_column, newsletter, social_links, copyright

#### Step 6: Homepage template

9. **`templates/index.json`** — Wire up the homepage template with the sections in order:
   - hero-video-banner
   - editorial-collection-grid
   - featured-product-slider
   - text-with-image-split

### Existing theme files to preserve

This project started from the Horizon theme by Shopify. Many existing files in `blocks/`, `snippets/`, `assets/` (JS files), and `templates/` are from Horizon and should NOT be deleted — they provide functionality (cart drawer, search modal, quick add, etc.) that the new sections may depend on.
